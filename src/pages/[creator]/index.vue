<template>
  <div>
    ...
    <SideDrawer
      :class="{ 'slide-in-menu': side_drawer }"
      :user="user"
      @closeDrawer="toggleDrawer"
    />
    <div
      @scroll="scrollingBody"
      id="creator-store-section"
      style="
        overflow-y: scroll;
        overflow-x: hidden;
        height: 100vh;
    ...
    >
      ...
      <div id="creator-store-section-hide">
        ...
        <div
          v-if="!isDesktop"
          class="top-bar pl-16 pr-16 flex-space-between pt-12 pb-16"
          id="top-bar"
        >
          <div v-if="user" class="flex-together gap-16">
            <img
              @click="toggleDrawer"
              src="@/assets/img/icons/ham-burg.svg"
              alt=""
            />
          </div>
          <div>
            <transition name="translate-title">
              <div
                v-if="
                  !page_title && (creator.name || creator.store_display_name)
                "
                style="display: flex; align-items: center; gap: 8px"
                class="creator-title page-title"
              >
                <div style="width: 32px; height: 32px">
                  <img
                    style="
                      ...
                      object-fit: cover;
                    "
                    :src="getReplacedSource(creatorProfilePic)"
                    :alt="creator.name"
                  />
                </div>
                <div
                  style="
                    max-width: calc(100vw - 28px - 138px - 24px - 40px - 16px);
                    white-space: nowrap;
                    overflow: hidden;
          ...
          <div style="display: flex; align-items: center; gap: 16px">
            <ExploreButton
              ...
              @click="goToSearch"
            />
            <div v-if="user" class="heart-icon">
              <img
                @click="router.push('/wishlist')"
                class="topbar-right-element"
            ...
            <div class="cart-icon" @click="router.push('/cart')">
              <div
                class="no-items header-no-items"
                v-if="cart_items && cart_items.length > 0"
              >
                {{ getCartItemsLength(cart_items) }}
              </div>
              <img
                class="topbar-right-element"
        ...
        <div v-if="creator">
          <div class="creator-info">
            ...
            <div class="creator-info-container desktop">
              ...
              <div class="creator-title-wrapper">
                <div class="creator-title-container">
                  ...
                  <button
                    v-if="user_following"
                    @click="unfollow_author"
                    class="following-btn active-btn"
                  >
                    Following
                  </button>
                  <button
                    v-else
                    @click="follow_author"
                    class="following-btn inactive-btn"
                  >
                    Follow
                  </button>
                </div>
            ...
            <div v-if="botdImages" class="sticky-container">
              <div class="botd ban-desk">
                ...
                >
                  <slide v-for="(botdImage, index) in botdImages" :key="index">
                    <div style="postion: relative">
                      <div
                        class="timer botd-timer"
                        v-if="checkSaleTimer(botdImage.redirectionUrl)"
                      >
                        Sale Ends in {{ timeLeft }}
                      </div>
                      <img
                        :src="getReplacedSource(botdImage.src)"
                        alt=""
                        class="botdImg"
                        @click="redirectToBWB(botdImage.redirectionUrl)"
                        :key="index"
                      />
                    </div>
                  </slide>
                ...
                <div
                  v-else-if="botdImages.length === 1"
                  style="position: relative"
                >
                  <div
                    class="timer"
                    v-if="checkSaleTimer(botdImages[0].redirectionUrl)"
                  >
                    Sale ends in {{ timeLeft }}
                  </div>
                  <img
                    :src="getReplacedSource(botdImages[0].src)"
                    alt=""
                    class="single-botdImg"
            ...
            <div v-else class="creator-contents">
              <div
              ...
              <div
                :class="{ 'active-creator-contents': active_tab == 'products' }"
                @click="changeTab('products')"
              >
                Spotlight
              </div>
              <div
                ...
                :class="{ 'active-creator-contents': active_tab == 'pebbles' }"
                @click="changeTab('pebbles')"
              >
                Pebbles
              </div>
            ...
            <!-- <div class="product-contents" v-if="active_tab == 'products'">
              <div
                :id="product.id"
                @click="saveProductScrollPosition(product.id)"
                v-for="(product, i) in catalog"
                :key="i"
              >
                <Product
                  class="product-card"
                  origin="creator_store"
                  source="creator_featured_products"
                  :itemInfo="product"
                />
              </div>
            ...
            <div class="product-contents" v-if="active_tab == 'pebbles'">
              <div v-for="(pebble, i) in pebbles" :key="i">
                <div class="pebbles-cover" @click="selectPebble(pebble.id)">
                  ...
                  <img
                    :src="thumbnail(pebble.media_info.thumbnail_url[0], 300)"
                    alt=""
                  />
                </div>
              </div>
            ...
            <div class="collection-contents" v-if="active_tab == 'collections'">
              <div
                ...
                :id="collection.id"
                @click="saveCollectionScrollPosition(collection.id)"
                :key="c"
              >
                <CollectionComponent
                  :item="collection"
          ...
          <!-- uncomment -->
          <!-- <Pebble
            v-if="selected_pebble_index != null"
            :user="user"
            :creator="creator || {}"
            :store="true"
            :wishlisted_items="wishlisted_items"
            :pebbles_list="pebbles"
            :selected_pebble_index="selected_pebble_index"
            :isDesktop="isDesktop"
            :is_followed_by_user="user_following"
            :play="play"
            :selected_pebble="selected_pebble"
            :muted="muted"
            @muteAudio="muteAudio"
            @autoPlay="autoPlay"
            @fetchMoreResults="fetchMoreResults"
            @closePebble="closePebble"
            @selectPebbleByIndex="selectPebbleByIndex"
              ...
              <div class="emoji">
                ...
                >
                  ...
                  <circle
                    ...
                    cy="16"
                    r="1.20312"
                    fill="url(#paint8_radial_884_1431)"
                  />
                  <circle
                    cx="14.957"
                    cy="16"
                    r="1.20312"
                    fill="url(#paint9_radial_884_1431)"
                  />
                  <circle
                    cx="24.832"
                    cy="22.9062"
                    r="1.20312"
                    fill="url(#paint10_radial_884_1431)"
                  />
                  <circle
                    cx="24.832"
                    cy="22.9062"
                    r="1.20312"
                    fill="url(#paint11_radial_884_1431)"
                  />
                  <circle
                    cx="24.832"
                    cy="22.9062"
                    r="1.20312"
                    fill="url(#paint12_radial_884_1431)"
                  />
                  <circle
                    cx="23.8242"
                    cy="7.01562"
                    r="1.20312"
                    fill="url(#paint13_radial_884_1431)"
                  />
                  <circle
                    cx="23.8242"
                    cy="7.01562"
                    r="1.20312"
                    fill="url(#paint14_radial_884_1431)"
                  />
                  <circle
                    cx="9.99219"
                    cy="3.36719"
                    r="1.20312"
                    fill="url(#paint15_radial_884_1431)"
                  />
                  <circle
                    cx="9.99219"
                    cy="3.36719"
                    r="1.20312"
                    fill="url(#paint16_radial_884_1431)"
                  />
                  <circle
                    cx="9.99219"
                    cy="3.36719"
                    r="1.20312"
                    fill="url(#paint17_radial_884_1431)"
                  />
                  <circle
                    cx="4.92969"
                    cy="8.10938"
                    r="1.20312"
                    fill="url(#paint18_radial_884_1431)"
                  />
                  <circle
                    cx="4.92969"
                    cy="8.10938"
                    r="1.20312"
                    fill="url(#paint19_radial_884_1431)"
                  />
                  <path
                  ...
                  <g filter="url(#filter6_f_884_1431)">
                    <path
                      ...
                      stroke="url(#paint37_linear_884_1431)"
                      stroke-width="0.3"
                  ...
                  <g filter="url(#filter7_f_884_1431)">
                    <path
                      d="M19.4727 3.01562C19.3893 4.52604 18.7352 8.09063 16.7852 10.2656"
                      stroke="url(#paint38_linear_884_1431)"
                      stroke-width="0.3"
                      stroke-linecap="round"
                    />
                  </g>
                  <defs>
                    ...
                    >
                      ...
                      <feBlend
                        mode="normal"
                        in2="shape"
                        result="effect1_innerShadow_884_1431"
                      />
                    </filter>
                    <filter
                      id="filter1_f_884_1431"
                      x="4.69922"
                      y="11.9131"
                      width="15.2031"
                      height="13.4214"
                    ...
                    >
                      ...
                      <feGaussianBlur
                        stdDeviation="0.25"
                        result="effect1_foregroundBlur_884_1431"
                      />
                    </filter>
                    <filter
                      id="filter2_f_884_1431"
                      x="5.83125"
                      y="12.3244"
                      width="13.4508"
                      height="11.7248"
...
// import FooterHypd from "../components/FooterHypd.vue";
...
import { optimizeImage, getReplacedSource } from "../../Helpers/helperMethods";
...
const emit = defineEmits(["loadingPage"]);
...
// to check if element is scrolled into view
...
const selected_pebble_index = ref();
const creator = ref({});
const pebbles = ref([]);
const catalog = ref([]);
...
const selected_pebble = ref();
const loadingPebbles = ref(false);
const loadingProducts = ref(false);
const loadingCollections = ref(false);
const total_no_of_catalogs = ref();
...
// Async For Loop
async function asyncForLoop(positionObj, method) {
  for (let i = 0; i <= positionObj.paginationCount; i++) {
    await this.getCollections(i);
  }
}
async function scrollToPosition() {
  isLoading.value = true;
  await asyncForLoop(this.$store.state.collectionScrollPosition, "collections");

  let element = document.getElementById(
    this.$store.state.collectionScrollPosition.collectionId
  );
...
function getElementIntoView(element) {
  let mainElement = document.getElementById("creator-store-section");

  // Changing the scroll behavior to auto, so user cannot see scrolling body
  mainElement.style.scrollBehavior = "auto";
  element?.scrollIntoView();

  // To set the proper position
  let adjustment;
  if (botd.value) {
    adjustment = 270;
  } else {
  ...
  mainElement.scrollTo(0, element?.offsetTop - adjustment);

  // Changing it back to smooth so the scroll To Top shows scrolling body
  mainElement.style.scrollBehavior = "smooth";

  // Stopped Loading after Element into view
  isLoading.value = false;
}

function saveProductScrollPosition(productId) {
  let scrollPositionObj = {
    productId,
    paginationCount: catalog.value.findIndex((item) => item.id == productId),
  };
  this.$store.dispatch("saveProductScrollPosition", scrollPositionObj);
}

function saveCollectionScrollPosition(collectionId) {
  let scrollPositionObj = {
    collectionId,
    paginationCount: pagination_count_collection.value,
  };
  this.$store.dispatch("saveCollectionScrollPosition", scrollPositionObj);
  tracking.trackingClickOnCreatorCollection(creator.value, collectionId);
}
...
function handleChange() {
  if (window.innerWidth > 480 && window.innerWidth < 900 && !this.isDesktop) {
    ...
    document.getElementById("creator-store-section").classList.remove("mobile");
    document
      .getElementById("creator-store-section-hide")
      .classList.add("mobile");
  } else {
    document.querySelector("#show-error").classList.add("show-panda");
    document.querySelector("#show-error").classList.remove("show-panda-true");
  }
}
...
function changeTab(options) {
  active_tab.value = options;
  router.replace({
    path: route.fullPath,
    query: { active: options },
  });
...
async function getPebbles() {
  ...
  query.append("influencer_id", creator.value.id);
  query.append("page", pagination_count_pebble.value);
  try {
    let response = await axios({
      method: "GET",
      url: this.$cmsURL + "/api/pebble/influencer",
      withCredentials: true,
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (response.data.payload && response.data.payload.length > 0) {
      let pebbles = [...this.pebbles, ...response.data.payload];
      this.pebbles = pebbles.filter(function ({ id }) {
...
async function getCatalogIds() {
  ...
  let query = new URLSearchParams();
  query.append("influencer_id", creator.value.id);
  query.append("page", pagination_count_product.value);

  try {
    let response = await axios({
      method: "GET",
      url: this.$cmsURL + "/api/catalog/influencer",
      withCredentials: true,
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (response.data.payload && response.data.payload.length > 0) {
      total_no_of_catalogs.value = response.data.payload.length;
      catalog.value_ids = response.data.payload;
      await getCatalog();
    } else {
...
async function getCatalog() {
  // This loading is for Scrolling the product into view
  if (
    this.$store.state.productScrollPosition &&
    !isLoading.value &&
    !scrolledIntoView.value
  ) {
    isLoading.value = true;
  }
  ...
  for (
    let i = catalog.values_sent + 0;
    i < catalog.values_sent + max_limit;
    i++
  ) {
    ...
    if (
      this.$store.state.productScrollPosition &&
      this.$store.state.productScrollPosition.paginationCount == i
    ) {
      scrollIntoViewCheck.value = true;
    }
  }
  ...
  try {
    let response = await axios({
      method: "GET",
      url: this.$catalogURL + "/api/v2/app/catalog/basic",
      params: params,
      withCredentials: true,
      headers: {
        "Content-Type": "application/json",
    ...
    var element;
    if (response.data.payload) {
      catalog.value = [...catalog.value, ...response.data.payload];
      catalog.values_sent += max_limit;

      if (scrollIntoViewCheck.value) {
        scrollIntoViewCheck.value = false;
        setTimeout(() => {
          element = document.getElementById(
            this.$store.state.productScrollPosition.productId
          );
          getElementIntoView(element);
          // Element scrolled into view, now loader condition will never get result to true
          scrolledIntoView.value = true;
        }, 0);
      }

      if (total_no_of_catalogs.value < 1) {
        // If there's no position saved, then loader will get false here
        if (isLoading.value) {
          isLoading.value = false;
        }
        return;
...
function selectPebble(id) {
  ...
  if (i != -1) {
    ...
    selected_pebble.value = this.pebbles[i];
    router.push({
      name: "pebble_page",
      params: {
    ...
    tracking.trackingClickOnCreatorPebble(user.value, creator.value, id);
  }
}
...
async function getInfluencerInfo(id) {
  try {
    let response = await this.axios({
      method: "GET",
      url: this.$entityURL + "/api/app/influencer/" + id,
      withCredentials: true,
      headers: {
        "Content-Type": "application/json",
      },
    ...
    parseSocialLinks(
      response.data.payload.external_links,
      response.data.payload.social_account
    );
    this.getPebbles();
    this.getCatalogIds();
...
async function getBOTD() {
  ...
  try {
    let response = await this.axios({
      method: "GET",
      url: this.$entityURL + "/api/app/bwb",
      withCredentials: true,
      headers: {
        "Content-Type": "application/json",
      },
    ...
    botd.value = response.data.payload;
    // checking device width after getting results of BOTD
    checkDevice();
  } catch (err) {
    console.log(Err);
  } finally {
...
async function getPebbleById(id) {
  ...
  try {
    let response = await this.axios({
      method: "GET",
      url: this.$cmsURL + "/api/pebble/id?id=" + id,
      withCredentials: true,
      headers: {
        "Content-Type": "application/json",
      },
    ...
    selected_pebble.value = response.data.payload;
    this.pebbles.push(response.data.payload);
    selected_pebble_index.value = 0;
    if (route.query.influencer_id) {
      getInfluencerInfo(route.query.influencer_id);
    } else {
      getInfluencerInfo(selected_pebble.value.influencer_ids[0]);
    }
  } catch (err) {
    console.log(err);
  } finally {
...
async function getCollections(i) {
  try {
    let response;
    if (i != undefined) {
      ...
      pagination_count_collection.value = i;
    } else {
      response = await this.axios({
        method: "GET",
        url:
    ...
    if (response.data.payload) {
      collections.value = [...collections.value, ...response.data.payload];
    } else {
      received_all_collection.value = true;
    }
    if (collections.value.length > 0 || Object.keys(route.query).length != 0) {
      settingActiveTab(true);
    } else {
      active_tab.value = "products";
    }
  } catch (err) {}
...
watch(route, (to, from) => {
  if (to.name == "pebble_page" && from.name == "pebble_page") {
    this.selectPebble(to.params.id);
  }
...
onMounted(() => {
  if (
    this.$store.state.searchedProductsInfo &&
    this.$store.state.searchedProductsInfo?.products?.length > 0
  ) {
    this.$store.dispatch("clearSearchProducts");
  }
  ...
  window.addEventListener(
    ...
    function () {
      if (window.orientation == 90) {
        ...
        document.querySelector(".panda").classList.remove("rotate-panda-right");
      } else if (window.orientation == -90) {
        document.querySelector(".panda").classList.add("rotate-panda-right");
        document.querySelector(".panda").classList.remove("rotate-panda-left");
      }
    },
    false
  );

  if (route.name == "pebble_page" || route.name == "lagacyPebble_page") {
    play.value = false;
